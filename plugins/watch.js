...
Tracker.Plugins.addOn( "watch", function(){
    var util, tmpl, template, window, document, log, time, startBtnId, stopBtnId, clearBtnId, 
        ...
        original__tracker__, new__tracker__, StatusPool, showState, arrivedSnippetGroupCache,
        buttonStateChange, logHtml;

    util = Tracker.util;
    tmpl = util.tmpl;
    StatusPool = Tracker.StatusPool;
    ...
    new__tracker__ = function( groupId ){
        ...
        if( code = StatusPool.snippetGroupToCodeGet( groupId ) ){
    ...
    start = function(){
        ...
        StatusPool.arrivedSnippetGroupFlagSet( 2 );
        showState( "监控进行中...", "#c00" );
        log( "监控开始" );
        hooking = true;
    ...
    stop = function(){
        ...
        StatusPool.arrivedSnippetGroupFlagSet( 1 );
        showState( "已停止" );
        log( "监控结束" );
    ...
    clear = function(){
        for( var groupId in arrivedSnippetGroupCache ){
            StatusPool.arrivedSnippetGroupDelete( groupId, 2 );
    ...
    eventBuild = function(){
        ...
        util.forEach( topNav.childNodes , function( li ){
    ...
    this.onStartUp( function( win, doc ){
        ...
        this.body.innerHTML = template.page( {
            ...
            startBtnId: startBtnId = util.id(),
            stopBtnId: stopBtnId = util.id(),
            clearBtnId: clearBtnId = util.id(),
            stateId: stateId = util.id(),
            logerId: logerId = util.id()
        } );
